...
class SemSegAugmentor_CV(object):
    ...
    def _shift_rot_zoom(self, _img, _mask, _center, _angle, _scale):
        ...
        img_trans = cv2.warpAffine(imgT, tRotMat, (cols, rows), flags=cv2.INTER_AREA,
                                   borderMode=cv2.BORDER_REFLECT_101)  # """,flags=cv2.INTER_CUBIC,"""
        mask_trans = cv2.warpAffine(maskT, tRotMat, (cols, rows), flags=cv2.INTER_AREA,
